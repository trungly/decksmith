...
| `contentSize` | `"XS" \| "S" \| "M" \| "L"` | `"M"` | How large slide body content feels. `XS` = smallest text / tightest spacing; `M` matches the former `S` scale; `L` matches the former `M`. Scales slide font-size, padding, and stack gap together; themes keep their individual typographic identities. |
